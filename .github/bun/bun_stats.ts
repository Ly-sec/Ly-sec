...
const pinnedReposQuery = config.pinned
  ...
  repo${index}: repository(owner: ${repo.owner ? `"${repo.owner}"` : "$username"}, name: "${repo.name}") {
    name
    description
    stargazerCount
    primaryLanguage {
...
export async function generateStats() {
  ...
  const pinned = config.pinned
    .map((configRepo, index) => {
      ...
      return {
        ...
        language: configRepo.language || repo.primaryLanguage?.name || "N/A",
  ...
  let growth = "0%";
  if (lastYearTotal > 0) {
    const raw = ((currentTotal - lastYearTotal) / lastYearTotal) * 100;
    if (raw > 500) {
      growth = (raw / 100).toFixed(1) + "x Growth";
    } else {
      growth = (raw > 0 ? "+" : "") + raw.toFixed(1) + "%";
    }
  }
